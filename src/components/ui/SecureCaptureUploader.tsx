...
const STEP_INSTRUCTION_KEYS: Record<string, string> = {
  id_front: 'capture.idFrontInstruction',
  id_back: 'capture.idBackInstruction',
  selfie: 'capture.selfieInstruction',
  item_photo: 'capture.itemPhotoInstruction',
}
    ...
    return (
      ...
      >
        ...
        <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--color-text-primary)' }}>
          {instruction || (STEP_INSTRUCTION_KEYS[step] ? t(STEP_INSTRUCTION_KEYS[step]) : t('capture.tapToOpenCamera'))}
        </span>
        <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>
          {t('capture.tapToActivateCamera')}
        </span>
      </div>
    )
  }
    ...
    <div style={{ position: 'relative', background: '#000', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: 80, left: 0, right: 0, textAlign: 'center' }}>
        <Typography.Text style={{ color: '#fff', fontSize: 14, background: 'rgba(0,0,0,0.5)', padding: '6px 16px', borderRadius: 20 }}>
          {instruction || (STEP_INSTRUCTION_KEYS[step] ? t(STEP_INSTRUCTION_KEYS[step]) : t('capture.takeAClearPhoto'))}
        </Typography.Text>
      </div>
